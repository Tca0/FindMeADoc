  ...
  try {
    ...
    if (patientFound) {
      return res
        .status(400)
        .json({
          message: `Patient with email ${newPatient.email} already exists`,
        });
    }
...
async function updatePatient(req, res, next) {
  ...
  try {
    ...
    if (!updatedPatient)
      return res.json({ message: "Patient not found by ID" });
    if (req.currentUser.role === "doctor")
      return res.json({ message: "Doctors cannot edit patients profiles" });
    if (req.currentUser.patientID !== id)
      return res.json({
...
async function removePatient(req, res, next) {
  ...
  try {
    if (req.currentUser.role === "doctor")
      return res.json({ message: "Doctors cannot remove user profiles" });
    if (req.currentUser.patientID !== id)
      return res.json({
